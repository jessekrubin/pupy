...
def new_package(relative_path):
    try:
        ...
        relative_path
        sstring(_path.join(relative_path, '__init__.py'),
                _PKG_INIT.strip("\n").strip("\r\n") + "\n")
        sstring(_path.join(relative_path, '__main__.py'),
                _PKG_MAIN.strip("\n").strip("\r\n") + "\n")
        sstring(_path.join(relative_path, 'cli.py'),
                _IO_PKG_CLI.strip("\n").strip("\r\n") + "\n")
        sstring(_path.join(relative_path, 'utils.py'),
                _PKG_UTILS.strip("\n").strip("\r\n") + "\n")
...
PARSER.add_argument(
    "-V", "--version",
    action="store_true",
    help="Print pupy version."
    )

NEW_SUBPARSER = SUBPARSERS.add_parser('new')
NEW_SUBPARSER.add_argument('path', type=unescaped_str, nargs=ZERO_OR_MORE, help="relative path"
                           )
